...
namespace DanceDanceRotationModule.NoteDisplay
{
    public class NotesContainer : Blish_HUD.Controls.Container
    {
        ...
        internal class WindowInfo
        {
            ...
            internal int PerfectNoteStartPosition { get; private set; }
            /** Where a note should be at when it is needs to be destroyed */
            ...
            {
                NoteHeight = (Height - 60) / 6;
                NoteWidth = NoteHeight;

                DestroyNotePosition = 0;
                PerfectNoteStartPosition = (int)(0.2 * Width);
                NewNoteXPosition = Width;

                NotePositionChangePerSecond = (NewNoteXPosition - PerfectNoteStartPosition) / 3.0;
            }
            ...
            {
                var YPos = 60 + NoteHeight / 12 + (Lane * NoteHeight);
        ...
        internal class ActiveNote
        {
            ...
            public ActiveNote(Note note, Image image)
            {
                this.Note = note;
                this.Image = image;
                this.XPosition = 0.0;
        ...
        public void Start()
        {
            ...
            _perfectStartLine = new Image(Resources.Instance.MugTexture)
            {
                ...
                Location = new Point(_windowInfo.PerfectNoteStartPosition, 0),
                Parent = this
            };
            _perfectEndLine = new Image(Resources.Instance.MugTexture)
            {
                ...
                Height = LineHeight,
                BackgroundColor = Color.White,
                Location = new Point(_windowInfo.PerfectNoteStartPosition + _windowInfo.NoteWidth, 0),
                Parent = this
            };
        }
        ...
        public void Update(GameTime gameTime)
        {
            ...
            for (int index = _info.ActiveNotes.Count - 1; index >= 0; index--)
            {
                ...
                activeNote.XPosition -= (_windowInfo.NotePositionChangePerSecond * (gameTime.ElapsedGameTime.Milliseconds) / 1000.0);
                ...
                {
                    if (activeNote.XPosition <= _windowInfo.PerfectNoteStartPosition)
                    {
                        // TODO: Mark it as missed
                    }
